...
class InviteTask extends PluginTask {
    ...
    */
    ...
    public function onRun($tick) 
    {
        ...
        if(isset($this->plugin->inv_requests[$this->player]))
        {
            if(($player = $this->plugin->getServer()->getPlayerExact($this->player)) instanceof Player)
            {
                $player->sendMessage($this->getPlugin()->getLanguage()->translateString("error.inv_timeout"));
            }
            unset($this->plugin->inv_requests[$this->player]);
        }
